...
command(/\b(?:twitter|trends?|topic|idea|theme)\b/i, function(msg)
{
    get('http://api.twitter.com/1/trends/current.json', function(data)
    {
        for ( time in data.trends )
            var trends = data.trends[time];

        msg.say('What\'s your opinion on ' + trends[Math.floor(Math.random()*trends.length)].name + '?');
    });
});
